...
Путь к вашей папке уже вставлен: `C:\Users\vadim\Downloads\sol\smanuals_datasheets`.
Менять больше ничего не нужно.

Это промт **первой** фазы: поднять проект, разобрать свою папку, замерить и
начать наращивать корпус. Когда корпус скачан, следующий промт —
`PROMPT-PHASE-C.md`: остановиться, посчитать, что реально получилось, и
поднять качество вместо количества.
