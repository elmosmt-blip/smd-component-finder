/* ============================================================================
 * ocr.js — camera, cropping, preprocessing and marking recognition
 * ----------------------------------------------------------------------------
 * Why this is harder than "hand the picture to tesseract":
 *
 *  1. An SMD marking is TINY text (0.3–1 mm) on a contrasting body. Tesseract
 *     sees nothing in a phone frame: the glyphs are 8–15 pixels tall. You have
 *     to crop and magnify 4–6x.
 *  2. Laser marking on black compound is LIGHT text on a DARK background —
 *     Tesseract expects the opposite. Fix: binarise + invert, try both
 *     polarities and keep whichever scores better.
 *  3. Glare, blur, low contrast. Fix: several preprocessing variants (Otsu
 *     threshold, local threshold, aggressive contrast stretch) and run each.
 *     The result is then fed to matcher, which knows the typical OCR slips
 *     (O<->0, I<->1, S<->5).
 *
 * The engine is tesseract.js, fetched over the network only on first use and
 * only inside the user's browser. Without OCR the tool is fully offline. Frames
 * never leave the machine except into the local WASM engine.
 * ========================================================================= */
(function (global) {
  'use strict';
  ...
  /**
   * Lazy-load tesseract.js and initialise the worker.
   * @param {function} onProgress — callback for the status line
   */
  function ensureWorker(onProgress) {
    ...
    if (state.loading) {
      return new Promise(function (resolve, reject) {
        ...
        setTimeout(function () { clearInterval(t); reject(new Error('OCR engine load timed out')); }, 60000);
      });
    }
    state.loading = true;
    return Promise.resolve()
      .then(function () {
        ...
        if (onProgress) onProgress('Loading the OCR engine (~10 MB the first time)…');
        return loadScript(TESSERACT_CDN);
      })
      .then(function () {
        if (!global.Tesseract) throw new Error('Tesseract did not load — check access to the CDN');
        if (onProgress) onProgress('Initialising recognition…');
        return global.Tesseract.createWorker('eng', 1, {
          logger: function (m) {
            if (!onProgress || !m) return;
            if (m.status === 'recognizing text' && m.progress != null) {
              onProgress('Recognising… ' + Math.round(m.progress * 100) + '%');
            } else if (m.status) {
              onProgress(m.status);
            }
          }
  ...
  function grayFromImageData(imgData) {
    ...
    for (var i = 0, g = 0; i < len; i += 4, g++) {
      // weighted luminance: eyes and cameras read green as brighter
      gray[g] = (d[i] * 0.299 + d[i + 1] * 0.587 + d[i + 2] * 0.114) | 0;
    }
    return gray;
  }

  /** Otsu's method: the classic histogram-based auto threshold. */
  ...
  /**
   * Local (adaptive) threshold: average over a block.
   * Saves frames with glare and uneven lighting — the usual macro problem.
   */
  function localThreshold(gray, w, h, blockSize, bias) {
    var half = blockSize >> 1;
    // integral image for a fast box average
  ...
  /** Contrast stretch by percentiles — removes the washed-out look. */
  ...
  /**
   * Builds a canvas from a binary array (0/255) with the requested inversion
   * and scale factor.
   * Tesseract works best at ~30–60 px glyph height, hence scale >= 4.
   */
  var MAX_DIM = 2000; // canvas size cap: tesseract slows badly on huge images
  ...
  function binaryToCanvas(binary, w, h, invert, scale, pad) {
    ...
    // paint the binary buffer through ImageData: per-row fillRect on a large
    // frame took seconds, this is a single pass over the array
    ...
    // cap the output size, otherwise recognition takes minutes
    ...
    octx.imageSmoothingEnabled = false; // nearest neighbour — glyph edges stay sharp
    octx.drawImage(small, 0, 0, iw, ih, 0, 0, out.width, out.height);
    return out;
  }

  /**
   * Auto crop: find the bounding box of "ink" (dark or light pixels) near the
   * centre of the frame, to cut away excess body and board background.
   */
  function autoCropRect(gray, w, h) {
    var thr = otsuThreshold(gray);
    // count which is rarer in the frame: assume the marking is the minority
    var dark = 0;
  ...
  function rectFromPreset(w, h, preset) {
    // preset: 'small' | 'wide' | 'large' — fraction of the frame
  ...
  /**
   * Builds the set of preprocessing variants for a crop of the frame.
   * Returns an array of { canvas, label, invert }.
   */
  function buildVariants(source, rect) {
    ...
    // baseline 2x magnification before analysis — on very small crops there
    // are too few pixels for a meaningful binarisation
    ...
    // 1. Otsu threshold, straight and inverted
    var b1 = otsuBinary(gray, w, h);
    out.push({ canvas: binaryToCanvas(b1, w, h, false, scale), label: 'Otsu (dark text)', invert: false });
    out.push({ canvas: binaryToCanvas(b1, w, h, true, scale), label: 'Otsu inverted (light marking)', invert: true });

    // 2. Contrast + Otsu — saves a washed-out frame
    var st = stretch(gray, 0.02, 0.02);
    var b2 = otsuBinary(st, w, h);
    out.push({ canvas: binaryToCanvas(b2, w, h, false, scale), label: 'contrast + Otsu', invert: false });
    out.push({ canvas: binaryToCanvas(b2, w, h, true, scale), label: 'contrast + Otsu inverted', invert: true });

    // 3. Local threshold — saves a frame with glare over half the body
    try {
      var b3 = localThreshold(st, w, h, Math.max(8, Math.min(48, Math.round(Math.min(w, h) / 6) | 1)), 4);
      out.push({ canvas: binaryToCanvas(b3, w, h, false, scale), label: 'local threshold', invert: false });
      out.push({ canvas: binaryToCanvas(b3, w, h, true, scale), label: 'local threshold inverted', invert: true });
    } catch (e) { /* local threshold is expensive on very large frames — skip it */ }
  ...
  /**
   * Runs OCR over the set of variants.
    ...
    return ensureWorker(onProgress).then(function (worker) {
      ...
      variants.forEach(function (v, idx) {
        chain = chain.then(function () {
          if (onProgress) onProgress('Variant ' + (idx + 1) + '/' + variants.length + ': ' + v.label);
  ...
  /**
   * Full cycle: frame -> crop -> preprocessing -> OCR -> cleanup.
  ...
  function analyze(source, opts, onProgress) {
    ...
    // 'manual' — the user dragged the box, the most reliable source there is
    var rect;
    if (opts.crop === 'manual' && opts.rect) {
      ...
      // auto crop is cheaper to compute on a downscaled copy of the frame
