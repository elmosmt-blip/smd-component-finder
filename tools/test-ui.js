...
check(' база сообщает о себе', /записей/.test(doc.getElementById('dbInfo').textContent));
check(' стартовое состояние — заглушка детали', /Выберите кандидата/.test(doc.getElementById('detailBody').textContent));
...
check(' в панели есть распиновка', /Pinout/.test(doc.getElementById('detailBody').textContent));
check(' коды маркировки видны', /BAV99|KJD/.test(doc.getElementById('detailBody').textContent));
check(' счётчик результатов заполнен', /Найдено/.test(doc.getElementById('resultsCount').textContent));

search('7O2');
check(' OCR-ошибка «7O2» находится как 2N7002', /2N7002/.test(cards()[0].textContent),
  cards()[0].textContent.slice(0, 60));
check(' стратегия поиска объясняет исправление', /исправлено/.test(doc.getElementById('strategy').textContent),
...
check(' подсказка по выбранному корпусу заполняется',
  /выв\./.test(doc.getElementById('pkgMeta').textContent),
...
search('');
check(' очистка запроса возвращает заглушку', /Выберите кандидата/.test(doc.getElementById('detailBody').textContent));
