...
def _serialized(cls):
    ...
    The web server keeps a single CardStore for the whole process and hands it
    to every HTTP worker thread. sqlite3 refuses a connection from a thread
    other than the one that opened it, so every public method runs under a
    lock instead of reconnecting on every request (300k rows make connection
    churn the most expensive thing on the site).
    """
...
class CardStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        # check_same_thread=False is deliberate: see _serialized above.
        con = sqlite3.connect(str(self.path), check_same_thread=False)
        con.row_factory = sqlite3.Row
        con.execute("PRAGMA journal_mode=WAL")
        # NORMAL survives a process crash and is fast enough for a bulk run;
        # SMD_SQLITE_SYNC=OFF trades power-loss safety for speed — only worth
        # it on a machine with a UPS, and only for the first, biggest pass.
        con.execute("PRAGMA synchronous=%s" %
                    (os.environ.get("SMD_SQLITE_SYNC") or "NORMAL"))
        # 300k cards means the writer and the site's readers meet constantly;
        # without a busy timeout the loser raises SQLITE_BUSY straight away.
    ...
    def conn(self) -> sqlite3.Connection:
        """The connection, reopened if the file was replaced underneath us.

        A long-lived process (the web server) can outlive a `rm cards.db`
        performed elsewhere; without this it would keep reading a deleted
        inode and report an empty database until restarted.
        """
        try:
            ino = self.path.stat().st_ino
        except OSError:
            ino = self._ino
        if self._conn is None or ino != self._ino:
            with self._lock:
    ...
    def iter_cards(self) -> List[dict]:
        """Every full card. Materialised, not a generator: the lock that makes
        the store thread-safe would be released the moment we yielded."""
        rows = self.conn.execute("SELECT card FROM cards").fetchall()
