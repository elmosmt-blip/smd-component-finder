            ...
            if not todo:
                ...
                with ProcessPoolExecutor(max_workers=self.jobs, initializer=_worker_init,
                    ...
                    for fut in as_completed(futures):
                        ...
                        # Commit in batches, but never sit on more than a
                        # minute of work: a 17-hour run should not lose hours
                        # to a power cut.
                        if st.done % 25 == 0 or time.time() - last_commit > 60:
                            store.commit()
                            last_commit = time.time()
...
class JobRunner:
    ...
    def start(self, paths: List[Path], shards: bool = False) -> str:
        # Shards are a static-hosting artifact: one JSON file per card. The
        # site itself reads SQLite, so a run started from the browser must not
        # pay for rewriting 300k files at the end of every job.
