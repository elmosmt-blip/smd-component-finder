...
# Recognising a table by its header is a vocabulary problem: the same pinout
...
TABLE_KINDS = [
    ("pins",
     ...
     ("name", "function", "description", "symbol", "type", "signal", "io", "i o")),
    ...
    # Do NOT add "unit" to electrical's second tuple: a ratings table is
    # "Symbol | Parameter | Value | Unit", and it would be stolen.
    ("electrical",
     ("symbol", "parameter", "characteristic"),
     ("min", "minimum", "typ", "typical", "max", "maximum", "test", "condition")),
    ("ratings",
     ("symbol", "parameter", "rating", "limit"),
     ("value", "max", "rating", "unit", "limit", "stress", "abs")),
    ("dimensions",
     ("dim", "dimension", "millimet", "inch", "mm", "size"),
     ("mm", "millimet", "inch", "min", "minimum", "max", "maximum",
      "nom", "nominal", "typ", "typical", "unit")),
    ("ordering",
     ("order", "part", "device", "type", "model"),
     ("package", "marking", "packing", "code", "number", "name")),
]
...
def _cell_has(cells: List[str], token: str) -> bool:
    """Does this token appear as a *word* in any header cell?

    `token in " ".join(cells)` is what this used to do, and it is quietly
    wrong twice over: "mm" matched "dummy", and — the one that actually bit
    — "mm" matched the part number MMBT3904 sitting in a merged header, so an
    ordering table came out as a dimensions table.

    Words only. A token matches a word exactly, as its plural, or (for tokens
    of four characters and up) as a stem: "dimension" covers "dimensions",
    "millimet" covers "millimeters". Short tokens stay exact, because every
    datasheet is full of part numbers that begin with two or three letters.
    """
...
def _shape_is_pins(table: List[List[str]]) -> bool:
    """A pinout with no header at all: 1 Base, 2 Emitter, 3 Collector.

    Plenty of datasheets draw the pin list as a plain two-column table with
    no header row, and pdfplumber hands it over exactly like that. The shape
    is unmistakable — the first column counts 1, 2, 3… — so it is recognised
    on shape instead of words. This is the fallback: a real header always
    wins, because the vocabulary pass runs first.
    """
    rows = [r for r in table if r and any((c or "").strip() for c in r)]
    if len(rows) < 3 or not (2 <= len(rows[0]) <= 4):
        return False
    numbers = []
    for row in rows:
        first = (row[0] or "").strip()
        if not re.fullmatch(r"[0-9]{1,3}[.)]?", first):
            return False
        numbers.append(int(first.rstrip(".)")))
    if numbers[0] != 1 or numbers != sorted(numbers):
        return False
    if len(set(numbers)) != len(numbers):          # 1,2,2 — not a pin list
        return False
    # the other columns are names, not prose or measurements
    others = [(c or "").strip() for row in rows for c in row[1:]]
    if not others or max(len(c) for c in others) > 48:
        return False
    return not any(re.fullmatch(r"[-+]?[\d.]+ ?[a-zA-Zµ%Ω]*", c) and
...
def classify_table(table: List[List[str]]) -> Optional[Tuple[str, Dict[str, int], int]]:
    ...
    for cells, rows_used in candidates:
        ...
        for kind, must, also in TABLE_KINDS:
            if not any(_cell_has(cells, m) for m in must):
                continue
            if not any(_cell_has(cells, a) for a in also):
                continue
            cols: Dict[str, int] = {}
            for i, c in enumerate(cells):
                ...
                for name, keys in {
                    ...
                    "param": ("parameter", "description", "function", "name",
                              "package", "characteristic", "signal"),
                    "value": ("value", "rating", "limit", "typ", "nom"),
                    "unit": ("unit", "mm", "inch", "millimet"),
                    "min": ("min",),
                    "max": ("max",),
                    "conditions": ("condition", "test"),
                    "pin": ("pin", "terminal", "pad", "ball", "lead", "conn"),
                    "marking": ("marking",),
                    "package": ("package",),
                    "order": ("order", "part", "device", "model"),
                    "packing": ("packing",),
                }.items():
                    if name in cols:
                        continue
                    if any(_cell_has([c], k) for k in keys):
                        cols[name] = i
                        break
    ...
    # No words we know. A pin list without a header is still a pin list.
    if _shape_is_pins(table):
        width = len(table[0])
        cols = {"pin": 0, "param": 1}
        if width >= 3:
            cols["description"] = 2
